-- HTTM Phase 1 — Bug fix: STRING_SPLIT yêu cầu SQL Server 2016+ và compatibility level ≥ 130.
-- DMPPortal legacy chạy ở compat level cũ hơn → 'Invalid object name STRING_SPLIT'.
--
-- Fix: thay STRING_SPLIT bằng inline TVF dùng XML PATH/nodes — chạy được mọi compat level từ
-- SQL Server 2005+. ALTER lại 2 SP đang dùng STRING_SPLIT (Search + GetMapData).

CREATE OR ALTER FUNCTION dbo.fn_Httm_SplitCsv (
    @Csv NVARCHAR(MAX)
)
RETURNS TABLE
AS
RETURN
(
    SELECT LTRIM(RTRIM(t.x.value('.', 'NVARCHAR(100)'))) AS value
    FROM (
        SELECT CAST(
            N'<x>' + REPLACE(ISNULL(@Csv, N''), N',', N'</x><x>') + N'</x>'
            AS XML
        ) AS d
    ) AS s
    CROSS APPLY s.d.nodes('/x') AS t(x)
);
GO
...
BEGIN
    ...
    WHERE f.Location IS NOT NULL
      ...
      AND (
          ...
          OR EXISTS (
              SELECT 1
              FROM dbo.fn_Httm_SplitCsv(@Types) AS s
              WHERE s.value <> N''
                AND f.HttmType = s.value
          )
      )
    ORDER BY f.UpdatedAt DESC, f.Id;
END;
